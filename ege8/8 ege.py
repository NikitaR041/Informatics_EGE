...
Третьим делом посмотри у тебя СЛОВО или ЧИСЛО:
    если число, то возможно нужно использовать continue для того,чтобы сказать, что число не начинается с нуля!
    если слово, то возможно нужно использовать set для избежания повтор букв (они просто поменяются местами и не заметно будет)
...
3) all(iterable) - такая функция, которая проверяет все итерабельные последовательные числа! СМ в НОМЕРЕ: 15
4) Допускается такое использование: (называется вхождение)
    if w[j] in 'СП': #это проверка на то, что есть ли такой элемент в итерации "СП", те есть ли элемент С или элемент П
'''
...
#    i = ''.join(i)
#    if (not 'УА' in i): # if i.count('Р') + i.count('С') + i.count('Т')+i.count('М') >= 3: при этом ответ 41984
#        c+=1
# print(c)
...
#    w = ''.join(w)
#    if  (w.count('Й') == 1 or w.count('Й') == 0) and w[0] != 'Й' and w[-1] != 'Й' and (not 'ЕЙ'  in w and not 'ЙЕ' in w):
#        c+=1
#     #    print(w)
# print(c)
...
1)Используй set в видим повторы букв - почему нужно?
    Потому что используется перестановка - есть шанс, что поменяются буквы, но так, как это для нас будет заметно, но для компьютера нет!
...
#12 - ДОМАШКА возможный Ответ 12600 неверно
'''
...
# a = set()
# c = 0
# for w in permutations('СПОРТЛОТО'):
#    w = ''.join(w)
#    if w[0] != 'О' and w[-1] != 'О':
#        a.add(w)
# for i in a:
...
#14 Ответ: надо узнать как решать эту задачу!!
'''
(PRO100 ЕГЭ) Иван составляет слова из букв Г, О, Л, соблюдая следующие ограничения:
1) две одинаковые буквы не могут стоять рядом;
2) буква Г может стоять только между буквами О и Л или Л и О; то есть, в начале слова и на конце буква Г
стоять не может; сочетания ОГЛ и ЛГО разрешены, сочетания ОГО и ЛГЛ запрещены. Сколько 20-буквенных
слов может составить Иван?

Идея:
1)Не могут быть ОО и ГГ и ЛЛ
2)Не могут быть ОГО и ЛГЛ и 'Г' не может стоять вначале и в конце, но может быть ОГЛ и ЛГО
'''
##from itertools import product
##c  = 0
##for w in product('гол', repeat = 20):
##    w = ''.join(w)
##    if 'ОО' not in w and 'ГГ' not in w and 'ЛЛ' not in w and w[0] != 'Г' and w[-1] != 'Г' and 'ОГЛ' in w  and 'ЛГО' in w :
##        c+=1
##print(c)
...
Ответ: 2961
№4696
Определите количество пятизначных чисел, записанных в восьмеричной системе счисления, в записи которых только одна цифра 6, при этом никакая нечётная
цифра не стоит рядом с цифрой 6.
'''
# from itertools import product
# k = 0
# w = ['16','36','56','76','61','63','65','67']
# for x in product('01234567', repeat = 5): # Берём только такие цифры, т.к. нужно 8-ричная запись числа
#     s = ''.join(x)
#     if s[0] != '0' and s.count('6') == 1 and all(c not in s for c in w):
#         k+=1
# print(k)

#16) 
""" 6591 Взято из пробника Петербурга сайта КЕГЭ
